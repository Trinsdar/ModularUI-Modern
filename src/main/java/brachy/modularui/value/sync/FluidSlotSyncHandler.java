...
public class FluidSlotSyncHandler extends ValueSyncHandler<FluidStack, FluidSlotSyncHandler> {
    ...
    public FluidSlotSyncHandler(IMultiTankFluidHandler fluidTank, int index) {
    ...
    private void tryClickContainer(MouseData mouseData) {
        ...
        if (!currentStack.getCapability(ForgeCapabilities.FLUID_HANDLER_ITEM, null).isPresent()) return;
            ...
            if (this.canFillSlot) {
                ...
                if (fluidHandlerItem != null) {
                    ...
                    if (!cellFluid.isEmpty()) {
                        if (cellFluid.isFluidEqual(currentFluid)) {
